...
struct MoveCommand<T: MessageIdentifier>: IMAPTaggedCommand {
    ...
    /// MOVE on large mailboxes (Gmail All Mail, mass selections) needs more
    /// than the protocol-default 5s. The server does a COPY-then-EXPUNGE
    /// under the hood; bursty traffic against Gmail or any provider with
    /// queueing routinely takes 10–20s to ACK. 30s mirrors the budget we
    /// already grant SELECT/STATUS/LIST.
